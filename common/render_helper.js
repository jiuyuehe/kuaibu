...
exports.UserRole = function(userRole){
  if(userRole === config.UserRole.NORMAL_USER){
      return  '客户';
  }else if(config.UserRoleAGENT_USER_LEVEL1<=userRole && userRole <= config.UserRoleAGENT_USER_LEVEL5){
      return  '代理账号';
  }else if(userRole === config.UserRole.TEAM_USER_CHILD){
      return  '团队子账号';
  }else if(userRole === config.UserRole.TEAM_USER){
      return  '团队账号';
  }else if(userRole === config.UserRole.SERVICE_USER){
      return  '客服';
  }else if(userRole === config.UserRole.ADMIN_USER){
      return  '管理员';
  }else if(userRole === config.UserRole.SUPER_USER){
      return  '超级管理员';
  }
}
